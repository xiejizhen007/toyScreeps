...
import { LogisticsNetwork, LogisticsNetworkRequest } from "Network/LogisticsNetwork";

// export interface Test1Memory {
//     transferTask: LogisticsNetworkRequest;
// }

/**
 * 仍存在迷惑行为
 */
export class Test1 extends Role {
    ...
    work(): void {
        ...
        if (this.memory.transferTask) {
            if (this.isWorking) {
                ...
                if (this.store[this.memory.transferTask.resourceType] != this.store.getUsedCapacity()) {
                    this.transferToCenter();
                    return;
                }

                if (this.memory.transferTask.target == 'any') {
                    ...
                    if (target) {
                        if (this.pos.isNearTo(target)) {
                            for (const resourceType in this.store) {
                                const amount = Math.min(this.store.getFreeCapacity(), target.store[resourceType]);
