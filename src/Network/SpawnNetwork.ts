    ...
    work(): void {
        for (const spawn of this.spawns) {
            if (!spawn.spawning) {
                ...
                if (request) {
                    const ret = spawn.spawnCreep(this.generateBody(request.setup), request.role + Game.time, {
                        memory: {role: request.role, room: this.room.name, isNeeded: false},
                    });

                    if (ret == OK) {
                        this.roomNetwork.memory.myCreeps.push(request.role + Game.time);
